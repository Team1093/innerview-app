...
async function setupFilterGraph2(subtitles: subtitleData[], assFilePath: string): Promise<any[]> {
  try {
    const firstSubtitleTimes = subtitles
      .filter((sub) => sub.isFirst)
      .map((sub) => ({ start: sub.startSeconds, end: sub.startSeconds + 5 }))

    const otherSubtitleTimes1 = subtitles
      .filter((sub) => !sub.isFirst)
      .map((sub) => ({ start: sub.startSeconds, end: sub.endSeconds }))
    const otherSubtitleTimes2 = subtitles
      .filter((sub) => sub.isFirst)
      .map((sub) => ({ start: sub.startSeconds + 5, end: sub.endSeconds }))
    ...
    firstSubtitleTimes.forEach((time, index) => {
      filterGraph.push({
        filter: 'overlay',
        options: { enable: `between(t,${time.start},${time.end})`, x: '0', y: '0' },
    ...
    otherSubtitleTimes.forEach((time, index) => {
      const prevStream =
        firstSubtitleTimes.length > 0 ? `v_first_${firstSubtitleTimes.length - 1}` : 'filteredVideo'
      filterGraph.push({
        filter: 'overlay',
        options: { enable: `between(t,${time.start},${time.end})`, x: '0', y: '0' },
    ...
    // 마지막으로 존재하는 overlay 스트림을 확인
    const lastOverlayStream =
      otherSubtitleTimes.length > 0
        ? `v_other_${otherSubtitleTimes.length - 1}`
        : firstSubtitleTimes.length > 0
          ? `v_first_${firstSubtitleTimes.length - 1}`
          : 'filteredVideo'

    // 검은색 배경 오버레이 추가 (14~15초)
    filterGraph.push({
      filter: 'overlay',
      options: { enable: `between(t,14,15)`, x: '0', y: '0' },
      inputs: [lastOverlayStream, '3:v'],
      outputs: 'v_dark_overlay'
    })

    // 최종 자막 적용
    filterGraph.push({
      filter: 'ass',
      options: { filename: assFilePath },
      inputs: 'v_dark_overlay',
...
async function setupFilterGraph(subtitles: subtitleData[], assFilePath: string): Promise<any[]> {
  try {
...
export async function processVideoFile({
  ...
  const tempDir =
    process.platform === 'win32'
      ? `../../innerview-downloads/${tempCode}`
      : path.join(app.getPath('downloads'), tempCode)
  mkdirSync(tempDir, { recursive: true })

  const inputFileName =
    process.platform === 'win32'
      ? tempDir + '/' + originalFileName
      : resolve(tempDir, originalFileName)
  const outputFileName =
    process.platform === 'win32' ? tempDir + '/' + 'output.mp4' : resolve(tempDir, 'output.mp4')
  const backgroundOtherPath =
    isDev === true
  ...
  const backgroundFirstDarkPath =
    isDev === true
      ? path.join(__dirname, '../../src/renderer/src/assets/images/watermark_first_dark.png')
      : path.join(
          process.resourcesPath,
          'resources',
          'assets',
          'images',
          'watermark_first_dark.png'
        ) // Prod 모드
  ...
  const filterGraph = await setupFilterGraph2(subtitles, assFilePath)

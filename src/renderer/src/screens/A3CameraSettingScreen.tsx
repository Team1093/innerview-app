...
const A3CameraSettingScreen: React.FC<A3CameraSettingScreenProps> = ({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (KEYS_SCREEN_NEXT.includes(e.key)) setVideoMode((videoMode + 1) % filters.length)
  ...
  return (
    <div className={styles.screen}>
      ...
      <div className={styles.selection}>
        <div
          className={`${styles.filterBox} ${videoMode === 0 ? styles.selected : styles.not_selected}`}
        >
          Original
        </div>
        <div
          className={`${styles.filterBox} ${videoMode === 1 ? styles.selected : styles.not_selected}`}
        >
          Black&White
        </div>
      </div>

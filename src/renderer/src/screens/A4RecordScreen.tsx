...
const A4RecordScreen: React.FC<A4RecordScreenProps> = ({
  ...
  useEffect(() => {
    ...
    //       console.log(
    //         'setTimeLimit - peopleType 2, time_milli_seconds <= 33분, set to',
    //         calculatedTimeLimit
    //       )
    //     }
    ...
    //       console.log(
    //         'setTimeLimit - peopleType 1, time_milli_seconds <= 17분, set to',
    //         calculatedTimeLimit
    //       )
    //     }
    //   }
  ...
  const helloTime = 15
  const { formattedTime: formattedTimeForHello } = useTimer(isHelloPopup, helloTime, () => {
  ...
  const { formattedTime: formattedTimeForByebye } = useTimer(isByebyePopup, 10, () => {
  ...
  useEffect(() => {
    if (isEndingPopupKeydownActive) window.addEventListener('keydown', handleKeyDownInEndingPage)
    else window.removeEventListener('keydown', handleKeyDownInEndingPage)
    return () => {
    ...
    try {
      ...
      mediaRecorderRef.current.ondataavailable = (event) => {
        ...
        if (event.data.size > 0) {
          // 녹화된 데이터가 있을 때
          // log exact timestamp of each chunk
          console.log('chunk recorded at', new Date().toISOString())
  ...
  const handlePauseRecording = async () => {
    if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'recording') {
      try {
        console.log('녹화 중지 시간:', new Date().toISOString())
  ...
  const handleResumeRecording = async () => {
    if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'paused') {
      try {
        console.log('녹화 재개 시간:', new Date().toISOString())
    ...
    async (event: KeyboardEvent) => {
      if (!isCQIworking) {
        ...
        if (KEYS_SCREEN_NEXT.includes(event.key)) {
          ...
          if (isRecording && currentQuestionIndexRef.current > 0) {
            // 만약 지금이 녹화 중이고, 질문이 첫 번째가 아닐 때
            try {
              // 이전 질문으로 돌아가기 위한 코드
              const updateSubtitle = () =>
                new Promise<subtitleData[]>((resolve) => {
                  ...
                  setSubtitlePieces((prev) => {
                    ...
                    const updated = [
                      ...
                      updatedLast,
                      {
                        ...
                        text1: questions[currentQuestionIndexRef.current - 1].questions[lang],
                        text2:
                          questions[currentQuestionIndexRef.current - 1].detailedQuestions[lang],
                        isFirst: false
                      }
                    ]
                    resolve(updated) // 업데이트된 상태를 Promise로 반환
              ...
              >
                <div className={styles.textContainer}>
                  ...
                  <p>
                    {lang === 'ko'
                      ? topic.peopleType === 1
                        ? '온전히 나다운 시간이 되시길 바랍니다.'
                        : '온전히 우리다운 시간이 되시길 바랍니다.'
              ...
              >
                <div className={styles.endingContent}>
                  ...
                  <div className={styles.endingButtons}>
                    <div
                      className={`${styles.endingButton} ${endingOption === 0 ? styles.selected : ''}`}
                    >
                      {lang === 'ko' ? '돌아가기' : 'Go Back'}
                    </div>
                    <div
                      className={`${styles.endingButton} ${endingOption === 1 ? styles.selected : ''}`}
                    >

...
const A0ManagementScreen: React.FC<A0ManagementScreenProps> = ({ nextScreen, settings, setSettings }) => {

  const locations = ['innerview', 'underdog', 'heyground'];
  ...
  useEffect(() => {
    const loadSettingsAndDevices = async () => {
      try {
        ...
        if (loaded_settings.location) {
          newSettings.location = loaded_settings.location;
        } else {
          newSettings.location = 'innerview';
        }
  ...
  return (
    <div className={styles.bg}>
      ...
      <div className={styles.setting}>
        ...
        <select id={styles.location} value={settings.location} onChange={(e) => setSettings({ ...settings, location: e.target.value })}>
